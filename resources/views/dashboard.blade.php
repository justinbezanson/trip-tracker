            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                ...
                <table class="min-w-full text-left text-sm font-light text-surface dark:text-white">
                    <thead class="border-b border-t border-neutral-200 font-medium dark:border-white/10 bg-cyan-700 text-white">
                        <tr>
                            <th width="100" class="px-6 py-4">Kid</th>
                            <th class="px-6 py-4">Trips</th>
                            <th class="px-6 py-4">Last Trip</th>
                        </tr>
                    </thead>
                    <tbody>
                        @foreach ($kids as $kid)
                            <tr class="border-b border-neutral-200 bg-slate-50 dark:border-white/10 hover:bg-slate-100">
                                <td class="whitespace-nowrap px-6 py-4 font-medium">{{$kid->name}}</td>
                                <td class="whitespace-nowrap px-6 py-4">{{count($kid->trips)}}</td>
                                <td class="whitespace-nowrap px-6 py-4">{{$kid->trips->last()->when}}</td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
                    ...
                    <tbody>
                        @foreach ($trips as $tripIndex => $trip)
                            ...
                            @endif
                                ...
                                <td class="px-6 py-4">{{$trip->where}}</td>
                                <td>
                                ...
                                <td class="px-6 py-4 text-right">
                                    <livewire:trips.delete-trip-button :trip="$trip" />
